...
| Vertical line pair | `news-start` / `news-end` | Each pair emits a `build-news` bundle. **Presence of any pair also adds `news` to the consolidated `06-close-on-reversal` alert's `inside_window`** — no extra flag. **Only lines anchored inside the chart's visible window are armed** — off-screen lines are treated as stale leftovers and ignored (an off-screen line whose window already closed would otherwise be rejected as a stale blackout). |
| Vertical line pair | `blackout-start` / `blackout-end` (or `pause` / `resume` aliases) | Each pair emits a `build-pause` bundle. Blocks entries while active. **Only lines anchored inside the chart's visible window are armed** — off-screen (e.g. past-window) lines are ignored as stale. |
