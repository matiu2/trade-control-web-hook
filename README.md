...
| *(none — automatic)* | take-profit resistance band | **H&S / iH&S only, default-on.** Every H&S trade also gets a one-sided band whose **far edge is pinned to the take-profit**, sitting just under TP for a long / just over for a short (width `--tp-resistance-pct`, default `0.1%` of TP). So a golden reversal candle printing as price nears TP flattens the position for a partial win instead of round-tripping to the stop. Unioned with any drawn `support`/`resistance` bands. Disable with `--skip-tp-resistance`. Not added on the M/W path (TP is recomputed live). |
...
cargo run -p tv-arm -- \
  ...
  --tp-resistance-pct 0.1 \           # width % of the automatic take-profit resistance band, far edge pinned to TP (H&S only, default 0.1)
