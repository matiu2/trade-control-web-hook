...
| Fib retracement | (label optional) | **Sets the trade direction**: the `0`-reading is the head (head above neckline → short H&S, head below → long IH&S). Which end is the head is read from TradingView's `reverse` flag — **not** the raw anchor order, which is unreliable (a fib drawn "neckline-first" still has its `0`-level at `points[1]`; trusting point order silently flipped AUD/CAD 2026-07 to a long). Also drives TP (`2 × neckline − head`) and the `pcl-exhausted` veto price (`midpoint + 0.8 × (TP − midpoint)`). Draw it spanning **head ↔ neckline** — orientation is taken care of by the level math, so click order doesn't matter. |
