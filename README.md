...
| `01-veto-too-low` | `veto` | Price crossing pcl-exhausted level | "Pattern completion level exhausted" — 80% of the way from the **neckline** to TP (= the **fib `1.8`** level on a `head(0)→neckline(1)` fib, since the `2.0` extension is TP). Value-bound, computed by `tv-arm` from the fib geometry. This matches the M/W `overshoot` level exactly (both `neckline + 0.8·(TP − neckline)`). (Was `midpoint + 0.8·(TP − midpoint)` ≈ fib `1.7` until 2026-07-20.) Direction-mirrors the invalidation veto, but level is `stop-next-entry`, **not** `close-positions`: a pcl breach is in the trade's favour (price ran toward TP), so it only blocks a *late* entry and never touches an open position. (Was wrongly `close-positions` until the trade-046 fix — it closed an in-profit short ~31 ticks early.) |
| `02-veto-trade-expiry` | `veto` | Vertical line crossing chart time | Hard stop: once the trade-expiry line passes, no more entries. |
| `03-prep-break-and-close` | `prep` | Trendline crossing (neckline break, close-confirmed) | Skippable for stocks / late entries with `--skip-break-and-close`. Fires **on the close**, using the **origin side** (2026-07-15): the side the plan started on is fixed by the open of the first bar it processed (the arm bar), and **any bar that closes on the far side of the neckline fires the break** — it does *not* require the below→above transition to happen on that exact bar. This is robust to a suppressed transition bar (a spread-hour rubbish candle) or arming already-past the line; the old edge detector lost those and stranded the plan in `AwaitBreakAndClose` (EUR/GBP, GBP/USD 2026-07). The close must still clear the far edge of the cross-buffer zone. The buffer is `cross_buffer_atr·ATR` — an **ATR fraction** (`tv-arm --cross-buffer-atr <f>`, e.g. `0.15` = 15% of a typical bar's range), which *self-scales with volatility* so one value works across EUR/GBP, Gold, indices. If the ATR can't be computed it degrades to 0 (fail-soft). A legacy `cross_buffer_pct` **percent-of-price** term is summed on top if set, but it's **deprecated** (default now 0, `--cross-buffer-pct` hidden + warns) — a fixed % of price is volatility-blind (0.1% is ~8.5p on EUR/GBP, far too wide for a close that broke by <1p), which is why the ATR unit replaced it. **Optionally golden-gated** (`tv-arm --bcr-require-golden`, off by default): with the flag on, the break bar must itself be *golden* — its full range (`high − low`) at least the Wilder ATR at that bar — so a weak, indecisive bar that merely closes past the neckline no longer stamps the break. This is **not** the signal-candle `--skip-golden` gate (which governs the enter's Pine reversal bar via `needs_golden`); a trendline-cross bar has no detector `golden` flag, so the engine measures the crossing bar's range vs ATR directly. Fails **closed** (bar treated as not-golden) if the window is too short to warm the ATR. |
| `04-prep-retest` | `prep` | Trendline crossing (intrabar retest of the neckline) | Skippable with `--skip-retest`. **Uses the same neckline drawing** as `03-prep-break-and-close` (opposite direction, intrabar) — you draw the neckline once. A separately-drawn `retest`/`neckline-retest`/`retrace` trendline is **ignored** (`tv-arm` logs and skips it): the retest *is* the neckline, and honouring a stale drawn line could arm a cross that never fires (USD/ZAR iH&S, 2026-07). Fires **intrabar on the wick**, not the close, and (as of 2026-07-03) not the open either: the bar's **high and low just have to straddle the neckline** — sit on opposite sides — with the directional wick reaching at/through the line (long retest = the low dips at/below the descending neckline). Open- and close-agnostic: a tap-and-bounce that opens *and* closes on the same side still counts — see CHANGELOG "intrabar cross is a pure straddle". **The retest zone fattens over time, scaled by the neckline's slope:** the first bar after the break-and-close must actually reach the neckline, and each subsequent bar widens the near-side band by `retest_atr_step × |neckline slope per bar|` (default step **0.075**, `tv-arm --retest-atr-step`), so a wick that comes *within* the growing tolerance of the line stamps the retest even without reaching it. A **horizontal neckline has zero slope → the band never widens** (retest it to the exact line — flat necklines are precise price levels); a **steeper neckline fattens the band faster**, because the reason a retest drifts from the line as bars pass is the line *moving away*, and how fast it moves is its slope (combined with volatility — the ATR the step is calibrated in cancels algebraically but still guards a fail-soft when the window is too short to warm it). See CHANGELOG "slope-scaled retest tolerance". The wick must also clear the cross buffer (`cross_buffer_atr·ATR`, plus a deprecated `cross_buffer_pct` term if set — see the break-and-close row) so a one-tick graze doesn't trip it. **Also golden-gated by `--bcr-require-golden`** (same flag as `03`): with it on, the retest bar must be golden (range ≥ ATR) as well as crossing — a low-range tap that reaches the line but has no real range doesn't stamp. See the break-and-close row for the full semantics. |
| `05-enter` | `enter` | Pine `Candle Signals` golden candle | The actual trade. Gated on the preps above + opposing-direction veto absent. Its stop-loss is normally **anchored** to the pattern extreme (`signal_high`/`signal_low`) + the ATR buffer; drop an **`sl` chart Note** to pin it to a level you drew instead — see "Fixed stop-loss from an `sl` chart Note" below. |
...
| `07-close-on-sr-reversal` | `close` | Pine `Candle Signals` opposing reversal | Emitted when `support`/`resistance` lines are drawn **or** (H&S / iH&S, default-on) an automatic take-profit resistance band is added — see `--tp-resistance-fib-level` / `--skip-tp-resistance`. Carries `inside_window: [price]` + `sr_bands: [[lo, hi], ...]`. **Armed independent of any news window** — a golden opposing reversal candle whose **pattern band anchor** sits in a band flattens the trade at close, in all trade types. The band test keys on the part of the pattern that *rejected off* the level (2026-07-22): an **engulfer** anchors on the **open of its first covered bar** (`N-1` — where the pair sat at the level before engulfing back out), a **pinbar/tweezer** on the print bar's **wick-50%** midpoint — a candle that merely *closed into* the band without originating at it (continuation) no longer trips it. See CHANGELOG "S/R reversal-close tests a pattern-aware band anchor". **Exit-only** — it flattens the position and nothing more. Defaults `needs_golden: true`. `tv-arm --veto-on-reversal` still stamps `veto_on_reversal: true` on this half but it's a **dormant no-op since 2026-07-19** (the worker no longer writes a `reversal` veto and no enter checks one). |
...
| `05-enter` | `enter` | Pine **`Every Bar Close`** alertcondition (every closed bar, not the golden/short-pattern plots) | Carries the baked static M/W params (`mw:` block); the **worker** computes entry/SL/TP from those + the live shell OHLC each bar, mid→bid/ask corrected with the arm-time spread. `max_retries: 0`, no preps. **`needs_golden: false`** — golden is a Head-and-Shoulders signal-candle gate; M/W entry is geometry-driven, so `--skip-golden` is irrelevant here. |
...
1. **Draw the setup on a TradingView chart.** Mark the invalidation line,
   ...
   `support`/`resistance` horizontals near key levels). Don't draw a separate
   `retest` trendline — the retest always reuses the neckline. A drawn
   `retest`/`neckline-retest`/`retrace` line is **ignored** (`tv-arm` logs it
   and moves on); support was dropped because a stale one could arm a
   never-firing cross (USD/ZAR iH&S, 2026-07).
...
**stop** at the geometry anchor. Override with one of the three mutually
exclusive flags: `--entry-market` (market on the signal bar), `--entry-stop`
(explicit default), `--entry-limit` (rest a limit, recovering to a stop when
wrong-side — see above). Not to be confused with the **position-tool**
`--market-entry` / `--stop-entry` / `--limit-entry` (note the reversed word
order), which read a drawn position tool instead of a pattern. `--strategy-v2`
conflicts with all three pattern flags (its two legs have fixed shapes). M/W's
own resolution is unaffected — it has no stop `EntrySpec`.
...
### Fixed stop-loss from an `sl` chart Note

By default the H&S / iH&S enter anchors its stop to the **pattern extreme**
(`signal_high` for a short, `signal_low` for a long) plus the ATR buffer.
To place the stop yourself — at the shoulder, or beyond the head — drop a
TradingView **Note** on the chart, write `sl` (or `stop-loss`) in it, and
put its **first anchor** at the price you want:

```
Note text:  sl
Anchor:     the shoulder / head level
```

The enter then carries that price instead of the anchored geometry. H&S
and iH&S only — M/W entries compute their geometry in the worker and
ignore the note.

**A buffer keeps the stop clear of the level.** The Note marks the wick;
a stop resting exactly *on* it is the one most likely to be clipped by
the same noise that formed the wick. So the drawn price is pushed away
from the pattern by `--sl-note-buffer-atr-pct` (default **0.5**, matching
the fraction the anchored SL already uses) — **up** for a short, **down**
for a long, direction taken from the fib. Pass `0` to use the drawn price
verbatim. The buffer is a *percent of ATR* resolved **at fire time**, not
at arm time, so it self-scales with volatility and a `--spec-in` re-arm
picks up current conditions rather than a frozen number.

**Which Note counts.** Charts accumulate Notes, so only one anchored
inside the setup's own window is read: from **5 bars before the fib's
earliest anchor** through the **trade-expiry** line. Outside that window a
Note is ignored and logged — it belongs to another setup. Inside it:

- **no `sl` Note** → the stop stays geometry-anchored (unchanged behaviour);
- **two or more** → the arm is **rejected** as ambiguous, never a
  latest-wins guess;
- **wrong side of the neckline** for the trade's direction → **rejected**,
  so a Note left over from an opposite-direction setup can't arm a "stop"
  on the profit side.

The label must be the *whole* Note text, so freeform annotations like
`sl too tight, moved it` are commentary, not a stop.
...
| Fib retracement | (label optional) | **Sets the trade direction**: the `0`-reading is the head (head above neckline → short H&S, head below → long IH&S). Which end is the head is read from TradingView's `reverse` flag — **not** the raw anchor order, which is unreliable (a fib drawn "neckline-first" still has its `0`-level at `points[1]`; trusting point order silently flipped AUD/CAD 2026-07 to a long). Also drives TP (`2 × neckline − head`) and the `pcl-exhausted` veto price (`neckline + 0.8 × (TP − neckline)` = the **fib `1.8`** level). Draw it spanning **head ↔ neckline** — orientation is taken care of by the level math, so click order doesn't matter. |
...
| *(none — automatic)* | take-profit resistance band | **H&S / iH&S only, default-on.** Every H&S trade also gets a band spanning the **take-profit** back to a **level on the drawn fib** (`--tp-resistance-fib-level`, default `1.8` — the pcl-exhausted level, the same price the `too-low`/`too-high` veto uses). So a golden reversal candle printing as price nears TP flattens the position for a partial win instead of round-tripping to the stop. Sized by the trade's own geometry, **not** by `--reversal-band-pct` (which sizes only *drawn* S/R lines) — the two shared one percent until 2026-08, so widenings meant for drawn lines silently inflated this band to 4× its intended size and flattened a EUR/NZD short 22.8 pips short of target. Unioned with any drawn `support`/`resistance` bands. Disable with `--skip-tp-resistance`. Not added on the M/W path (TP is recomputed live). |
...
cargo run -p tv-arm -- \
  ...
  --tp-resistance-fib-level 1.8 \     # far edge of the automatic TP resistance band, as a fib level (2.0 = TP, 1.8 = pcl-exhausted; H&S only, default 1.8)
  --skip-tp-resistance \              # do not add the automatic take-profit resistance band (default: added on H&S trades)
