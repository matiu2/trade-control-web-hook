...
| `04-prep-retest` | `prep` | Trendline crossing (intrabar retest of the neckline) | Skippable with `--skip-retest`. Fires **intrabar on the wick**, not the close: the bar must open on one side of the neckline and trade through to the other (long retest = open above the descending neckline, low dips at/below it). A tap-and-bounce that closes back on the original side still counts — see CHANGELOG "intrabar cross reads the wick". |
