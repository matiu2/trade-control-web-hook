...
| `03-prep-break-and-close` | `prep` | Trendline crossing (neckline break, close-confirmed) | Skippable for stocks / late entries with `--skip-break-and-close`. Fires **on the close**, using the **origin side** (2026-07-15): the side the plan started on is fixed by the open of the first bar it processed (the arm bar), and **any bar that closes on the far side of the neckline fires the break** — it does *not* require the below→above transition to happen on that exact bar. This is robust to a suppressed transition bar (a spread-hour rubbish candle) or arming already-past the line; the old edge detector lost those and stranded the plan in `AwaitBreakAndClose` (EUR/GBP, GBP/USD 2026-07). The close must still clear the far edge of the `cross_buffer_pct` zone (default 0.02% of the line price), so a close only just past the bare line can still be rejected as noise — lower it with `tv-arm --cross-buffer-pct <pct>` (or `0` for the bare line). |
