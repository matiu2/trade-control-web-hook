...
| backend | `main` @ `c96eea8` | 2026-06-30 | Version `2df395bc`. Activates **break-even stop** + **spread-blackout widen** (previously dormant) on the amend path now demo-verified (TN **v0.11.0** no-TP fix). Also carries the close-on-reversal / news-close / strategy-v2 fixes merged to `main` since the last dev deploy. |
