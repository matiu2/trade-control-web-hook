...
pub struct ReplayArgs {
    ...
    /// reversal-closes dropped) — the grid's **exit** axis.
    ///
    /// Like `--arm-skip-calendar-bars`, it must be passed explicitly because it
    /// is **not inferable** from the saved plan: a plan with no
    /// `07-close-on-sr-reversal` rule could equally mean "reversals were
    /// skipped" or "the chart had no S/R lines drawn and the TP resistance band
    /// was off".
    #[arg(long, requires = "save")]
