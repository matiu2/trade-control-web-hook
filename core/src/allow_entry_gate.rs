...
//!
//! The gate is pulled out of the worker's `dispatch_enter` so it stays
//! sync and small (no `worker::Env`, no async, no broker handle), which is
//! both easier to read and easier to test in isolation.
//!
//! Lives in `core` so **both** the live worker and the offline replay
//! (`engine::simulator`) apply the same `allow_entry` script + candle
//! gate; the worker re-exports it so its call sites are unchanged. Rhai
//! compiles off-wasm, so the replay can run the gate identically.

use crate::candle_gate::{self, CandleGateOutcome};
use crate::intent::{Intent, Resolved, Shell};
